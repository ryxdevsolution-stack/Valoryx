const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/webPrintService-Cv8FIhKm.js","assets/vendor-BojPTacb.js"])))=>i.map(i=>d[i]);
import{t as q,u as De,b as Y,j as e,e as re,R as he,a as pe,X as Ae,_ as ye,C as ze,B as Be}from"./index-wvhhUEPh.js";import{u as Ee,d as Fe,r as p,L as Le}from"./vendor-BojPTacb.js";import{D as Me,B as Re}from"./DashboardLayout-BHnQ4LF5.js";import{C as Ie,T as Ue}from"./SkeletonLoader-ryPC6-lI.js";import{g as Qe}from"./shopSettingsService-C0GPa5el.js";import{R as He}from"./rotate-ccw-DVwWMV3X.js";import{C as Ge}from"./calendar-j4qohiC7.js";import{F as be}from"./file-text-B3AnOzvv.js";import{U as We}from"./user-DaBT3cu5.js";import{C as Oe}from"./clock-ClavHTdD.js";import{W as fe,S as Ye}from"./wallet-_HFPGE4X.js";import{P as qe}from"./trending-up-G9IBnkb1.js";import{D as Ve}from"./dollar-sign-DZvnd2Va.js";import"./search-DsGH3vz8.js";import"./loader-circle-WeUo5YIA.js";import"./truck-Bwvlim6Z.js";import"./chevron-right-DelVm2fM.js";import"./zap-B0fBthkv.js";import"./triangle-alert-Dt5Wh5bE.js";function T(n){return"₹"+n.toLocaleString("en-IN",{minimumFractionDigits:2,maximumFractionDigits:2})}function Ke(n){const c=new Date(n),l=c.toLocaleDateString("en-IN",{day:"2-digit",month:"short",year:"numeric"}),f=c.toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit",hour12:!0});return`${l} | ${f}`}function Je(n){try{const c=JSON.parse(n);if(Array.isArray(c))return c.map(l=>l.payment_name||l.payment_type||"Cash").join(" + ")}catch{}return n||"Cash"}function Xe(n){return!n||n.length<4?n:"••••••"+n.slice(-4)}function b(n){return n?n.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;"):""}async function Ze(n){try{const c=await fetch(n,{mode:"cors"});if(!c.ok)return null;const l=await c.blob();return await new Promise((f,k)=>{const _=new FileReader;_.onloadend=()=>f(_.result),_.onerror=k,_.readAsDataURL(l)})}catch{return null}}async function et(n,c){const l=n.type==="gst",f=l?n.final_amount:n.total_amount,k=n.payment_status==="pending",_=Je(n.payment_type);let U=null;c.logo_url&&(U=await Ze(c.logo_url));const se=b(c.client_name.charAt(0).toUpperCase()),V=U?`<img src="${U}" alt="Logo" class="logo-img" />`:`<div class="logo-letter">${se}</div>`,Q=[c.address,c.address2].filter(Boolean).map(b).join(", "),P=[],C=[];c.gstin&&P.push(`<div class="biz-row"><span class="biz-lbl">GSTIN</span><span class="biz-val">${b(c.gstin)}</span></div>`),C.push(`<div class="biz-row"><span class="biz-lbl">Legal Name</span><span class="biz-val">${b(c.client_name)}</span></div>`),c.phone&&P.push(`<div class="biz-row"><span class="biz-lbl">Phone</span><span class="biz-val">${b(c.phone)}</span></div>`),c.email&&C.push(`<div class="biz-row"><span class="biz-lbl">Email</span><span class="biz-val">${b(c.email)}</span></div>`),n.customer_gstin&&P.push(`<div class="biz-row"><span class="biz-lbl">Cust. GSTIN</span><span class="biz-val">${b(n.customer_gstin)}</span></div>`),C.push(`<div class="biz-row"><span class="biz-lbl">Bill Type</span><span class="biz-val">${l?"GST Invoice":"Invoice"}</span></div>`);const H=`
    <div class="biz-grid">
      <div class="biz-col">${P.join("")}</div>
      <div class="biz-col">${C.join("")}</div>
    </div>`,K=n.customer_name&&n.customer_name!=="Walk-in Customer"?b(n.customer_name.split(" ")[0]):null,J=K?`Hi ${K}, here's your bill!`:"Here's your bill!",i=n.customer_phone?Xe(n.customer_phone):null,A=l?'<th class="tc gst-th">GST %</th>':"",w=n.items.some(y=>Number(y.discount_percentage||0)>0),z=w?'<th class="tc">Disc %</th>':"",j=n.items.map((y,R)=>{const L=l?`<td class="tc muted">${y.gst_percentage}%</td>`:"",Z=Number(y.discount_percentage||0),oe=w?`<td class="tc muted">${Z>0?`${Z}%`:"−"}</td>`:"";return`
      <tr class="${R%2===0?"":"row-alt"}">
        <td class="item-td">
          <span class="item-name">${b(y.product_name)}</span>
          ${y.item_code?`<span class="item-code">${b(y.item_code)}</span>`:""}
        </td>
        <td class="tc">${y.quantity}</td>
        <td class="tr">${T(y.rate)}</td>
        ${oe}
        ${L}
        <td class="tr fw">${T(y.amount)}</td>
      </tr>`}).join(""),B=n.discount_amount||0,v=n.negotiable_amount||0,N=[];if(N.push(`<tr><td class="tot-lbl">Subtotal</td><td class="tot-val">${T(n.subtotal||f)}</td></tr>`),B>0){const y=n.discount_percentage?`Discount (${n.discount_percentage}%)`:"Discount";N.push(`<tr><td class="tot-lbl">${y}</td><td class="tot-val green">− ${T(B)}</td></tr>`)}else v>0&&N.push(`<tr><td class="tot-lbl">Negotiated</td><td class="tot-val green">− ${T(v)}</td></tr>`);l&&(N.push(`<tr><td class="tot-lbl">CGST</td><td class="tot-val">${T(n.cgst)}</td></tr>`),N.push(`<tr><td class="tot-lbl">SGST</td><td class="tot-val">${T(n.sgst)}</td></tr>`));const E=c.receipt_footer?`<p class="footer-note">${b(c.receipt_footer)}</p>`:"",M=n.points_earned&&n.points_earned>0?`<div class="points-panel">
         ...
         </div>`:"",x=k?'<div class="pending-banner">⏳ &nbsp;PAYMENT PENDING — NOT YET COLLECTED</div>':"",ne=`<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"/>
<meta name="viewport" content="width=device-width,initial-scale=1.0"/>
<title>Invoice #${n.bill_number} — ${b(c.client_name)}</title>
<style>
/* ── Reset ─────────────────────────────────────────── */
...
<div class="card">
  ...
  <div class="hdr">
    ...
    <div class="location-block">
      ...
      <span class="view-store">${b(c.client_name)}</span>
    </div>
  </div>
  ...
  <div class="amount-hero">
    <div class="amount-row-top">
      <div>
        ...
        <div class="amount-payment">${b(_)}</div>
      </div>
  ...
  <div class="bill-meta-row">
    ...
    ${i?`<span class="meta-pill">Mobile <strong>${b(i)}</strong></span>`:`<span class="cashier-note">Cashier: ${b(n.user_name||"Admin")}</span>`}
  </div>
  ...
  <div class="totals-section">
    <table class="tot-tbl">
      ...
      <tr class="grand-row">
        ...
        <td class="grand-val">${T(f)}</td>
      </tr>
    </table>
  </div>

  <!-- Footer note -->
  ${E?`<div class="card-footer">${E}</div>`:""}
