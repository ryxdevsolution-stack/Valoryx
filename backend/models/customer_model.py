...
class Customer(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            'total_spent': round(float(self.total_spent), 2) if self.total_spent and str(self.total_spent).lower() != 'nan' else 0.00,
            'total_amount': round(float(self.total_spent), 2) if self.total_spent and str(self.total_spent).lower() != 'nan' else 0.00,
            'gst_bills': 0,
            'non_gst_bills': 0,
            'loyalty_points': self.loyalty_points or 0,
            'last_purchase': self.last_purchase_date.isoformat() if self.last_purchase_date else None,
            'first_purchase': self.first_purchase_date.isoformat() if self.first_purchase_date else None,
