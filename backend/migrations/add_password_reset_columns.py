...
Date: 2026-02-25
Columns: reset_token (VARCHAR 64, indexed), reset_token_expires (TIMESTAMP)
Run: python -m migrations.add_password_reset_columns [--sqlite] [--supabase] [--both]
"""
import os
import sys
import logging
...
def run_sqlite(sqlite_path=None):
    ...
    with engine.connect() as conn:
        if not column_exists(engine, 'users', 'reset_token'):
            conn.execute(text('ALTER TABLE users ADD COLUMN reset_token VARCHAR(64) NULL'))
            conn.execute(text('CREATE INDEX IF NOT EXISTS ix_users_reset_token ON users(reset_token)'))
            conn.commit()
            logger.info('Added users.reset_token')
        else:
            logger.info('users.reset_token already exists — skipping')

        if not column_exists(engine, 'users', 'reset_token_expires'):
            conn.execute(text('ALTER TABLE users ADD COLUMN reset_token_expires TIMESTAMP NULL'))
            conn.commit()
            logger.info('Added users.reset_token_expires')
        else:
            logger.info('users.reset_token_expires already exists — skipping')
...
def run_supabase(db_url=None):
    ...
    with engine.connect() as conn:
        ...
        conn.execute(text(
            'ALTER TABLE users ADD COLUMN IF NOT EXISTS reset_token_expires TIMESTAMP WITH TIME ZONE NULL'
        ))
        conn.commit()
        logger.info('Added users.reset_token and users.reset_token_expires')
...
if __name__ == '__main__':
    ...
    if not args or '--both' in args:
        ...
        print('Usage: python -m migrations.add_password_reset_columns [--sqlite] [--supabase] [--both]')
        sys.exit(1)
