...
interface PlasmaWaveProps {
  ...
  /** React to the cursor: parallax follow + intensify on hover. */
  interactive?: boolean;
}
...
export default function PlasmaWave(props: PlasmaWaveProps) {
  ...
  useEffect(() => {
    ...
    // --- Cursor interactivity (parallax + hover intensify) ---
    // Track on window + hit-test the canvas rect so it works even when the
    // layer is behind page content (pointer-events: none).
    const mouse = { x: 0, y: 0, inside: 0 };
    const onPointerMove = (e: PointerEvent) => {
      const r = ctn.getBoundingClientRect();
      const mx = (e.clientX - r.left) / r.width;
      const my = (e.clientY - r.top) / r.height;
      mouse.inside = mx >= 0 && mx <= 1 && my >= 0 && my <= 1 ? 1 : 0;
      mouse.x = mx - 0.5;
      mouse.y = my - 0.5;
    };
    if (interactive) {
    ...
    // Eased state so motion is smooth rather than snapping to the cursor.
    let easeX = 0;
    let easeY = 0;
    let easeHover = 0;
    ...
    const update = (now: number) => {
      ...
      // Ease toward the cursor target (parallax) and hover intensity.
      const targetX = interactive ? mouse.x * mouse.inside * 280 : 0;
      const targetY = interactive ? -mouse.y * mouse.inside * 280 : 0;
      const targetHover = interactive ? mouse.inside : 0;
      easeX += (targetX - easeX) * 0.06;
      easeY += (targetY - easeY) * 0.06;
      easeHover += (targetHover - easeHover) * 0.05;

      uniformOffset[0] = xOff + easeX;
      uniformOffset[1] = yOff + easeY;
      program.uniforms.iTime.value = (now - startTime) * 0.001;
      program.uniforms.uRotation.value = (rot * Math.PI) / 180;
      // Hover deepens the focal length and speeds the flow for a reactive feel.
      program.uniforms.uFocalLength.value = fLen + easeHover * 0.28;
      program.uniforms.uSpeed1.value = s1 * (1 + easeHover * 3.5);
      program.uniforms.uSpeed2.value = s2 * (1 + easeHover * 3.5);
